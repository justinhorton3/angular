...
export class AnimationTimelineContext {
  ...
  constructor(
      public element: any, subInstructions: ElementInstructionMap, public errors: any[],
...
export class AnimationTimelineBuilderVisitor implements AnimationAstVisitor {
  ...
  private _visitSubInstructions(
    ...
    const rootElement = context.element;
    ...
    if (startTime != furthestTime) {
      ...
      const delay = 0;
      const parentTimings = {duration: furthestTime - startTime, delay, easing: ''};

      const keyframes = [{offset: 0}, {offset: 1}];
      const parentInstruction = createTimelineInstruction(
  ...
  visitTiming(ast: AnimationTimingAst, context: AnimationTimelineContext): AnimateTimings {
    if (ast instanceof DynamicAnimationTimingAst) {
      const strValue = interpolateStyleLocals(ast.value, context.locals, context.errors);
  ...
  visitQuery(ast: AnimationQueryAst, context: AnimationTimelineContext) {
    ...
    let sameElementTimeline: TimelineBuilder = null;
    elms.forEach((element, i) => {

      context.currentQueryIndex = i;
      const innerContext = context.createSubContext(element);
